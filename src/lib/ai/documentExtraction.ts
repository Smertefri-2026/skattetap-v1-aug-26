...
export const DOCUMENT_EXTRACTION_INSTRUCTIONS = [
  ...
  "Du finner ALDRI på informasjon som ikke finnes i teksten. Mangler et felt, utelater du det i stedet for å gjette.",
...
const documentExtractionSchema = z.object({
  ...
  parties: z.array(z.string().min(1).max(200)).max(6),
  amounts: z
...
const RESPONSE_SCHEMA_HINT = `Svar med et JSON-objekt på nøyaktig denne formen (utelat felt du ikke finner grunnlag for -- ikke gjett):
{
