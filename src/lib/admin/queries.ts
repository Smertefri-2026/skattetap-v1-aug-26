...
export async function getAdminOverview(): Promise<AdminOverview> {
  ...
  const [
    ...
    supabase.from("refund_requests").select("id", { count: "exact", head: true }).eq("status", "open"),
  ]);
...
export async function listRefundRequests(): Promise<AdminRefundRequestRow[]> {
  const supabase = createAdminClient();
  const [{ data: requests }, emailByUserId] = await Promise.all([
    supabase
      .from("refund_requests")
      .select(
        "id, user_id, purchase_id, case_id, reason, status, admin_note, created_at, updated_at, purchases(amount_kr, product_code, products(name)), cases(title)"
      )
      .order("created_at", { ascending: false }),
    getUserEmailMap(),
  ]);
  ...
  return (requests ?? []).map((r) => {
    const purchase = r.purchases as unknown as {
      amount_kr: number;
      product_code: string;
    ...
    return {
      ...
      caseTitle: (r.cases as unknown as { title: string } | null)?.title ?? "(slettet sak)",
      productName: purchase?.products?.name ?? purchase?.product_code ?? "(ukjent produkt)",
      amountKr: purchase?.amount_kr ?? 0,
